...
mod binary {
    ...
    pub fn main() {
        ...
        let kernel_val = match env::var("KERNEL") {
            Ok(kernel) => kernel,
            Err(_) => {
                eprintln!(
        ...
        match kernel_val.as_str() {
            "aarch64" => {
                // println!("cargo:rustc-link-search=native=dependencies/lib");
                // println!("cargo:rustc-link-lib=static=optee");
            }
            _ => {
                let kernel = PathBuf::from(kernel_val);
            ...
            Kernel executable at `{}`\n", kernel.display());
                ...
                if cfg!(feature = "uefi_bin") {
                    ...
                    let mut file =
                        File::create(file_path).expect("failed to create kernel_info.rs");
                ...
                if cfg!(feature = "bios_bin") {
                    ...
                    let kernel_archive =
                        out_dir.join(format!("libkernel_bin-{}.a", kernel_file_name));
                    let mut cmd = Command::new(&objcopy);
                    ...
                    if cfg!(target_arch = "x86_64") {
                        ...
                        cmd.arg("-O").arg("elf64-littleaarch64");
                        cmd.arg("--binary-architecture=aarch64");
                    } else {
                        panic!("unexpected arch found");
                    }
                    ...
                    let ar = llvm_tools
                        ...
                        .unwrap_or_else(|| {
                            ...
                            eprintln!(
                                "so try updating your toolchain if you're using an older nightly"
                            );
                            process::exit(1);
                        });
                ...
                let config_stream = match env::var("KERNEL_MANIFEST") {
                 ...
                 Please use `cargo builder` for building.");
                    ...
                    Ok(path)
                        if Path::new(&path).file_name().and_then(|s| s.to_str())
                            != Some("Cargo.toml") =>
                    {
                        let err = format!(
                ...
                file.write_all(
                    quote::quote! {
                        ...
                        /// Public so that `bin/x86_64_uefi` can read framebuffer configuration.
