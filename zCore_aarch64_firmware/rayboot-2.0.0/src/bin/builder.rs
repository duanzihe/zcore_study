...
use argh::FromArgs;
use rayboot::disk_image::create_disk_image;
use std::{
    convert::TryFrom,
    fs::{self, File},
    io::{self, Seek},
...
struct BuildArguments {
    ...
    /// figure out the architecture, either x86_64 or aarch64
    #[argh(option)]
    arch: String,
    ...
    /// define target device for aarch64 only
    #[argh(option)]
...
fn main() -> anyhow::Result<()> {
    ...
    if args.firmware.uefi() {  //要求命令行传入fireware
        let build_or_run = if args.run { "run" } else { "build" };
        let mut cmd = Command::new(env!("CARGO"));
        cmd.arg(build_or_run);
        cmd.arg("--timings");
        ...
        if args.arch.eq("x86_64") {
            cmd.arg("--bin").arg("x86_64_uefi");
            cmd.arg("--target").arg("x86_64-unknown-uefi");
        //要求命令行输入arch
        } else if args.arch.eq("aarch64") {  //Rust 编译器会使用与该目标相对应的工具链来生成适合于 UEFI 环境的通用 ARM64 二进制文件
            cmd.arg("--bin").arg("aarch64_uefi");
            cmd.arg("--target").arg("aarch64-unknown-uefi");
        } else {
            return Err(anyhow!("unexpected architecture: {}", args.arch));
        }
        //添加uefi_bin，arm64， virt这三个特性到features
        let mut add_features = "uefi_bin".to_string();   //注意！
        if args.arch.eq("aarch64") {
            add_features.push_str(" arm64");
            match &args.target_dev {
                Some(dev) => {
                    add_features.push(' ');
                    add_features.insert_str(add_features.len(), dev);
                }
                _ => {
                    add_features.push(' ');
                    add_features.insert_str(add_features.len(), "virt");
                }
            }
        } else {
            add_features.push_str(" x64");
        }

        cmd.arg("--features")
            .arg(args.features.join(" ") + add_features.as_str());
        ...
        if args.arch.eq("x86_64") {
            ...
            cmd.env(
                ...
                &args
                    .kernel_manifest
                    .as_ref()
                    .expect("--kernel-manifest not set"),
            );
        } else if args.arch.eq("aarch64") {
            cmd.env("KERNEL", "aarch64"); //设置环境变量KERNEL=aarch64
        } else {
            return Err(anyhow!("unexpected architecture: {}", args.arch));
        }
        ...
        // Retrieve binary paths 获取二进制文件的路径
        cmd.arg("--message-format").arg("json");
            //cmd.output会调用执行之前构建的命令，并等待其完成。它会返回一个包含命令执行结果的 Output 结构体
        let output = cmd
            .output()
        ...
        let executable_name = executable_path
            .file_stem() //file_stem() 方法用于获取文件名的主干部分，即去掉文件扩展名的部分。
            .and_then(|stem| stem.to_str()) // 尝试将文件名转换为字符串切片（&str），这个过程可能失败（例如，文件名不是有效的 UTF-8 编码），因此使用 and_then() 处理可能的 Option 类型。
            .ok_or_else(|| {
                anyhow!(
        ...
        //根据提供的输出目录创建一个 UEFI 磁盘映像文件
        if let Some(out_dir) = &args.out_dir {
            let efi_file = out_dir.join(if args.arch.eq("aarch64") {
                format!("bootaa64.efi")
            } else {
                format!("boot-{}.efi", executable_name)
            });
    ...
    if args.firmware.bios() {
        ...
        if args.arch.eq("x86_64") {
            cmd.arg("--bin").arg("x86_64_bios");
            cmd.arg("--target").arg("x86_64-bootloader.json");
        } else {
            return Err(anyhow!("unexpected architecture: {}", args.arch));
        }

        let mut add_features = "bios_bin".to_string();
        if args.arch.eq("aarch64") {
            add_features.push_str(" arm64");
            match &args.target_dev {
                Some(dev) => {
        ...
        cmd.arg("-Zbuild-std=core,compiler_builtins,alloc");
        cmd.arg("-Zbuild-std-features=compiler-builtins-mem,compiler-builtins-mem");
        if let Some(target_dir) = &args.target_dir {
            cmd.arg("--target-dir").arg(target_dir);
        }
        if args.quiet {
